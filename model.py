...
class EconomyModel(Model):
    ...
    def add_transaction(self, transaction):
        ...
        self.G.add_edges_from([
            (transaction.sender.unique_id, transaction.receiver.unique_id,
    ...
    def get_graph_data(self):
        ...
        text = [f"Amount: {edge[2]['amount']}" for edge in edges]
        return x, y, text

    def step(self):
        self.schedule.step()
        self.central_bank.update_inflation_rate([
            agent.price for agent in self.schedule.agents 
    ...
    def distribute_employment(self, initial_employment_rate=0.6):
        ...
        for firm in active_firms:
            max_employees = min(firm.production_capacity, int(firm.capital / (firm.wage * 3)))
